...
final class SmartPredicateTests: TestCaseModel {
    ...
    private func makeElement(url: URL) -> PlaylistElement {
        PlaylistElement(mafDescription: MetaAudioFileDescription(url: url))
    }

    // MARK: - isDirty

    @Test func isDirtyMatchesDirtyElement() throws {
        var element = makeElement(url: TestBundleResources.shared.tabla_wav)
    ...
    @Test func isDirtyDoesNotMatchCleanElement() throws {
        let element = makeElement(url: TestBundleResources.shared.tabla_wav)
